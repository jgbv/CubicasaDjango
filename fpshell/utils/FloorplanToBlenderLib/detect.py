...
import json         #to avoid having to propagate across scripts
import os
# TODO: detect windows
# TODO: detect doors
...
def detectOuterContours(detect_img, imgInfo, output_img = None, color = [255, 255, 255]):
    """
    ...
    #modify to extract points:
    outputname = os.path.join("outputs", "POLYFLOOR-"+os.path.basename(imgInfo[0]).split(".")[0]+".json")
    jsonDict = {
        "imagePath": os.path.basename(imgInfo[0]), 	#string, filename of the image
        "imageHeight": imgInfo[1],			#integer, height in pixels
        "imageWidth": imgInfo[2],			#integer, width in pixels
        "predictions": [			#array of objects representing each polygon
                {
                    "confidence": 0.98,	#float
                    "class": "entirefloor",		#string, label class
                    "name": "Main",	#string, identifier for this polygon, optional
                    "points": []
                },
        ]
    }
    ...
    for i,j in enumerate(n):
        if(i%2==0 and i+1 < len(n)):
            point = {"x": int(n[i]), "y": int(n[i+1])}
            jsonDict["predictions"][0]["points"].append(point)
